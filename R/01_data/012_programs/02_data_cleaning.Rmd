...
Countries are ranked by source (admin data > survey > no data) and frequency.
Score on this indicator has a max of 1 point with 0.5 points for source and 0.5 points for frequency.  Detailed scoring for source and frequency components are below.

Source (0.5 points total):
...
0.333 Points. 2 values available within past 5 years; 	
0.167 Points. 1 values available within past 5 years; 	
0 Points. None within past 5 years
...
for (reference_year in 2016:2018) {
  temp <-D3.3.AKI.MORT %>%
          mutate(available=((reference_year-as.numeric(date))<=5) & (reference_year>as.numeric(date))) %>% #filter out years outside reference window of 3 years     
          mutate(D3.CHLD.MORT=if_else(available==TRUE,1,0)) %>% #create 0,1 variable for whether data point exists for country
          group_by(country) %>%
          summarise_all((~(if(is.numeric(.)) sum(., na.rm = TRUE) else first(.)))) %>% #group by country to create one observation per country                 containing whether or not data point existed
          mutate_at(.vars=c('D3.CHLD.MORT'), ~case_when(
            .>=3 ~ 1,
            .==2 ~ 0.6,
            .==1 ~ 0.3,
            .==0 ~ 0, 
          ...
          mutate(date=reference_year) %>%
          select( country, date, c('D3.CHLD.MORT')) 
