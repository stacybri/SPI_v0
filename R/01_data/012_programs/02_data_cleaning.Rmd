...
for (reference_year in 2016:2018) {
  temp <-wbstats::wb(country="countries_only", 
          ...
          ungroup() %>%
          mutate(SPI.D3.AKI=100*rowSums(.[aki_list$id], na.rm=T)/length(aki_list$id)) %>%
          select(iso3c, country, date, SPI.D3.AKI, aki_list$id) %>%
...
#Now loop from 2016 and 2018, keeping just data inside last 5 years.
for (reference_year in 2016:2018) {
  temp <-D3.3.AKI.MORT %>%
          mutate(frequency=((reference_year-as.numeric(date))<=5) & (reference_year>as.numeric(date))) %>% 
          mutate(SPI.D3.CHLD.MORT.FREQ=if_else(frequency==TRUE,1,0)) %>% #create 0,1 variable for whether data point exists for country
          mutate(SPI.D3.CHLD.MORT.QUAL=case_when(
          ...
          group_by(country) %>%
          summarise(SPI.D3.CHLD.MORT.FREQ=sum(SPI.D3.CHLD.MORT.FREQ, na.rm=T),
                    SPI.D3.CHLD.MORT.QUAL=max(SPI.D3.CHLD.MORT.QUAL, na.rm=T)) %>% 
          mutate(SPI.D3.CHLD.MORT.FREQ=case_when(
            SPI.D3.CHLD.MORT.FREQ>=3 ~ 0.5,
            SPI.D3.CHLD.MORT.FREQ==2 ~ 0.3,
            SPI.D3.CHLD.MORT.FREQ==1 ~ 0.15,
            SPI.D3.CHLD.MORT.FREQ==0 ~ 0, 
            TRUE ~ 0
          )) %>%
          mutate(SPI.D3.CHLD.MORT=SPI.D3.CHLD.MORT.FREQ+SPI.D3.CHLD.MORT.QUAL) %>% 
          mutate(date=reference_year) %>%
          select( country, date, starts_with('SPI.D3.CHLD.MORT')) 


  assign(paste("D3.3.AKI",reference_year,sep="_"), temp)
}

D3.3.AKI <- bind_rows(D3.3.AKI_2016, D3.3.AKI_2017, D3.3.AKI_2018)
...
### AKI 3.5: People using safely managed drinking water services (% of population) 
...
msc_sumstats <- D3.AKI %>%
  group_by(date) %>%
  select(SPI.D3.AKI, paste('SPI.D3.',aki_list$id, sep='')) %>%
  skim() %>%
  select(c("skim_variable",
           "date", "complete_rate", "numeric.mean",  "numeric.sd",    "numeric.p0",    "numeric.p25",   "numeric.p50",
           "numeric.p75",   "numeric.p100",  "numeric.hist"))
...
D4.DPO <- D4.1.SC.DPO.CALD %>%
  left_join(D4.2.SC.DPO.NADA, by=c('iso3c', 'date')) %>%
  left_join(D4.3.SC.DPO.PORT, by=c('iso3c', 'date')) %>%
  left_join(D4.4.SC.DPO.TIME, by=c('iso3c', 'date')) %>%
  left_join(D4.5.SC.DPO.META, by=c('iso3c', 'date')) %>%
  left_join(D4.6.SC.DPO.USER, by=c('iso3c', 'date')) %>%
  left_join(D4.7.SC.DPO.GEOS, by=c('iso3c', 'date')) %>%
  select(iso3c, country.x, date, starts_with("SPI.D4")) %>%
  rename(country=country.x)
